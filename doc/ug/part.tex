...
\begin{essyntax}
  ...
  \require{2,6,8,12}{\opt{magn\_aniso\_energy \var{Ema}}}
  \begin{features}
...
\item[\opt{magn\_aniso\_energy \var{Ema}}] Sets the energy of the 
  uniaxial magnetic anisotropy of the particle. The default is 0.
\end{arguments}
...
Variant \variant{14} enables uniaxial magnetic anisotropy in all particles where 
corresponding parameter $magn_aniso_energy\neq0$ and the directions of the anisotropy axes
are determined by the coordinates of particle director \var{quatu}. 
...
The use of variant \variant{12} requires the description for a 
magneto-active particle of both magnetic dipole moment and anisotropy 
energy along with the direction of the easy axis. The letter is 
associated with the director of the particle, i.e. is set by the value 
of \var{quatu} property. To specify the dipole moment it is necessary 
to create a virtual particle, using the VS\_RELATIVE framework, which will be related to corresponding 
non-virtual particle and will be situated at the same position. 

\minisec{Example}
The first (non-virtual) particle possess anisotropy energy and axis parallel 
to the \var{Ox} and the related virtual particle carries dipole 
magnetic moment with value of 2.0 and oriented to direction of 
\var{Oy}:
\begin{code}
  constraint magnetic_anisotropy
  
  part 0 magn_aniso_energy 1.0 quatu 1 0 0
  part 1 virtual 1 vs_related 0 0 dip 0 1 0
  part 1 dipm 2.0
\end{code}

Magnetic anisotropy affect the rotation of the particles by the addition 
torques due to the interaction between dipole moment and anisotropy axis, 
which is described in uniaxial case by the potential:
\begin{equation}
U_{ma}=-E_{ma}\(\vec{e}\cdot\vec{n}\)^2,
\end{equation}  
here $\vec{e}$ and $\vec{n}$ are unit vectors of the directions of 
particle magnetic dipole moment and anisotropy axis respectively.
