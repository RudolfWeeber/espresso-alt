...
import espressomd.code_info as code_info
import numpy as np
...
#    self.pid=particleId
  ...
  def setUp(self):
    self.system.bondedInter[0]=FeneBond(k=1,d_r_max=5)
    self.system.bondedInter[1]=FeneBond(k=1,d_r_max=5)
  ...
  def generateTestForVectorProperty(_propName,_value):
    ...
    def func(self):
      ...
      print(propName,value,getattr(self.system.part[self.pid],propName))
      self.assertTrue(self.arraysNearlyEqual(getattr(self.system.part[self.pid],propName), value),propName+": value set and value gotten back differ.")
  ...
  def generateTestForScalarProperty(_propName,_value):
    ...
    def func(self):
      ...
      setattr(self.system.part[self.pid],propName,value)
      print(propName,value,getattr(self.system.part[self.pid],propName))
      self.assertTrue(getattr(self.system.part[self.pid],propName)==value,propName+": value set and value gotten back differ.")

    return func


  test_00_pos=generateTestForVectorProperty("pos",np.array([0.1,0.2,0.3]))
